...
function ControllerStep({ refresh }: { refresh: () => Promise<void> }) {
  const [entityId, setEntityId] = useState(""); const [passphrase, setPassphrase] = useState(""); const [repeat, setRepeat] = useState("");
  ...
  const generate = async () => { try { if (passphrase !== repeat) throw new Error("The passphrases do not match."); const value = await generateControllerKey(entityId, passphrase); setPrivatePackage(value.privatePackage); setPublicPackage(value.publicPackage); setPrivateDownloaded(false); setPublicDownloaded(false); setReimported(false); setMessage("The encrypted controller key was generated locally. Download both packages, then reselect the private package."); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Key generation failed"); } };
  const importPrivate = async (event: ChangeEvent<HTMLInputElement>, initial: boolean) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; try { const value = JSON.parse(await file.text()) as ControllerPrivatePackage; const loaded = await loadControllerKey(value, passphrase); setPrivatePackage(value); setPublicPackage(loaded.publicPackage); setReimported(true); if (initial) setPrivateDownloaded(true); setMessage("The encrypted package and passphrase were verified locally."); } catch (caught) { setReimported(false); setMessage(caught instanceof Error ? caught.message : "The encrypted package could not be opened."); } };
  const activate = async () => { if (!privatePackage || !publicPackage) return; setBusy(true); setMessage("Establishing controller identity…"); try {
    ...
    const challenge = begun.challenge as Record<string, unknown>; const proof = await signControllerRegistration(privatePackage, passphrase, challenge);
    ...
    setPassphrase(""); setRepeat(""); setPrivatePackage(null); await refresh();
  } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Controller identity setup failed"); } finally { setBusy(false); } };
  return <Card className="space-y-5 p-6"><div className="flex gap-3"><KeyRound className="text-blue-600" /><div><h2 className="text-xl font-semibold">2. Controller identity</h2><p className="text-sm text-gray-500">The private key remains encrypted under controller custody. The Server receives only the public identity, a possession proof, and your exact root-passkey approval.</p></div></div>
    <label className="block text-sm font-medium">Passphrase (at least 16 characters)<input type="password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-600 dark:bg-gray-900" /></label>
    {!privatePackage && <div className="grid gap-4 md:grid-cols-2"><section className="rounded-lg border border-blue-200 p-4 dark:border-blue-800"><h3 className="font-semibold">Generate locally <span className="text-sm font-normal text-blue-600">Recommended</span></h3><p className="my-3 text-sm text-gray-500">Opaque identity: {entityId || "Preparing…"}</p><label className="block text-sm">Repeat passphrase<input type="password" value={repeat} onChange={(event) => setRepeat(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-600 dark:bg-gray-900" /></label><Button className="mt-3" disabled={passphrase.length < 16 || passphrase !== repeat} onClick={() => void generate()}>Generate encrypted key</Button></section><section className="rounded-lg border border-gray-200 p-4 dark:border-gray-700"><h3 className="font-semibold">Import encrypted package</h3><p className="my-3 text-sm text-gray-500">Use a package made by the independent Evidence-Public controller-key tool.</p><label className="inline-flex min-h-11 cursor-pointer items-center rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium dark:border-gray-600">Select encrypted package<input type="file" accept="application/json,.json" className="sr-only" disabled={passphrase.length < 16} onChange={(event) => void importPrivate(event, true)} /></label></section></div>}
    {privatePackage && publicPackage && <><div className="rounded-lg border border-gray-200 p-4 text-sm dark:border-gray-700"><p><strong>{publicPackage.entity_id}</strong> · {publicPackage.key_id}</p><p className="mt-1 break-all font-mono text-xs">SHA-256 {publicPackage.public_key_sha256}</p></div><div className="flex flex-wrap gap-3"><Button variant="outline" onClick={() => { downloadJson(`${publicPackage.key_id}.controller-key.json`, privatePackage); setPrivateDownloaded(true); }}><Download size={17} />Encrypted private package</Button><Button variant="outline" onClick={() => { downloadJson(`${publicPackage.key_id}.controller-public.json`, publicPackage); setPublicDownloaded(true); }}><Download size={17} />Public package</Button><label className="inline-flex min-h-11 cursor-pointer items-center rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium dark:border-gray-600">Recovery-test private package<input type="file" accept="application/json,.json" className="sr-only" onChange={(event) => void importPrivate(event, false)} /></label></div><Button disabled={!privateDownloaded || !publicDownloaded || !reimported || busy} onClick={() => void activate()}>Verify and approve controller identity</Button></>}
