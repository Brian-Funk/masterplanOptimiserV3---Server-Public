...
The old `v3.4.0` release is retired. Do not reuse or replace its tag. The current prepared stable release is `v3.9.3`. Development deployments continue to use commit-addressed unsigned test artefacts and must show that they are not a production release.
