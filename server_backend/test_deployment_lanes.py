...
def test_release_version_is_synchronised_with_current_deployment_docs() -> None:
    ...
    assert backend == frontend == lock_document["version"] == "3.9.18"
    assert lock_document["packages"][""]["version"] == "3.9.18"
    assert "## 3.9.18" in text("changes.md")
    assert "signed-deployment.sh v3.9.18" in text("docs/deployment.md")
    assert "current prepared stable release is `v3.9.18`" in text("docs/publication-runbook.md")
