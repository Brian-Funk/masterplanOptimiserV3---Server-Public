...
ALTER TABLE events ADD COLUMN purge_grace_days INTEGER;
ALTER TABLE events ADD COLUMN purge_due_at TIMESTAMPTZ;
ALTER TABLE events ADD COLUMN purge_case_request_id VARCHAR(36);
ALTER TABLE events ADD COLUMN purge_started_at TIMESTAMPTZ;
...
CREATE INDEX ix_events_purge_due_at ON events(purge_due_at);
CREATE UNIQUE INDEX ix_events_purge_case_request_id
    ON events(purge_case_request_id) WHERE purge_case_request_id IS NOT NULL;

ALTER TABLE deletion_cases
    ADD COLUMN initiation_reason VARCHAR(32) NOT NULL DEFAULT 'authenticated_request';
ALTER TABLE deletion_cases ADD COLUMN event_purge_key VARCHAR(36);
...
CREATE UNIQUE INDEX ix_deletion_cases_event_purge_key
